...
function Features() {
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (e) => {
      ...
      setLineDataa(
        {
          labels: Object.keys(parsedData[0]).map((key) => (key)),
          datasets: [{ 
            label: sheetName,
            data: parsedData.map((row, index) => (Object.values(row).map((value, index) => (value))))
          }]
        }

      );
      setLabels(Object.keys(parsedData[0]).map((key) => (key)))
      setDatas(parsedData.map((row, index) => (Object.values(row).map((value, index) => (value)))));
    };
